...
export default function Analytics() {
  ...
  if (role !== "admin") {
    return <div className="clb-screen"><OneClubPageHeader eyebrow="Operations" title="Analytics" description="Privacy-safe Club Services usage insights." /><AccessDenied icon={ShieldCheck} title="Analytics access is restricted" reason="Your workspace does not include admin analytics." /></div>;
  }
  ...
  return (
    <div className="clb-screen">
      ...
      />
      {query.isLoading ? <OneClubLoadingState title="Loading analytics" message="Counting recent Club Services activity." /> : query.isError ? (
        <OneClubStateCard icon={BarChart3} title="Could not load analytics" message={errorMessage} tone="danger" />
      ) : query.data ? (
        <>
          <div className="clb-metric-grid">
            <OneClubMetricCard title="Active Users" value={query.data.active_users} tone="navy" />
            <OneClubMetricCard title="Discovery Views" value={query.data.features.club_discovery_view || 0} tone="green" />
            <OneClubMetricCard title="Join Requests" value={query.data.operations.join_requests_started || 0} tone="gold" />
            <OneClubMetricCard title="Event Check-ins" value={query.data.operations.event_check_ins || 0} tone="red" />
          </div>
