...
const sheetVariants = cva(
  "fixed z-50 gap-4 border-2 border-foreground bg-card p-6 shadow-[8px_8px_0_hsl(var(--foreground))] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute right-4 top-4 border-2 border-foreground bg-background p-1 shadow-[2px_2px_0_hsl(var(--foreground))] ring-offset-background transition-transform data-[state=open]:bg-secondary hover:-translate-y-0.5 hover:translate-x-0.5 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-4 w-4" />
