...
interface AdminEventCardProps {
  ...
  onInspect: (event: AdminEventRecord) => void;
...
export function AdminEventCard({
  ...
  const rsvpPercentage = Math.min(Math.round((event.rsvpsCount / event.capacity) * 100), 100);
  ...
  return (
    ...
    >
      <div className="space-y-3.5">
        {/* Card Header: Club Tag & Lifecycle Badge */}
        ...
        {/* RSVP Summary & Verified Attendance */}
        <div className="space-y-1.5 pt-1 text-xs">
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-muted-foreground">
              Confirmed RSVPs: <strong className="text-foreground">{event.rsvpsCount}</strong> / {event.capacity}
            </span>
        ...
        {/* Post-Event Compliance Status (Submitted vs Missing) */}
        {isPast && (
          <div className="pt-1">
            {event.postEventReport.status === "submitted" ? (
              <div className="flex items-center gap-1.5 text-[11px] font-medium text-emerald-700 dark:text-emerald-300 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/20">
                ...
                <span>Post-Event Report Submitted &amp; Audited</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 text-[11px] font-medium text-amber-700 dark:text-amber-300 bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/20">
                ...
                <span>Post-Event Compliance Report Missing</span>
              </div>
            )}
          </div>
        )}
      ...
      {/* Card Actions (No Complete/Cancel buttons) */}
      <div className="mt-4 pt-3 border-t border-border/70 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <Button
            ...
            className="h-8 gap-1 text-xs hover:border-primary hover:text-primary"
